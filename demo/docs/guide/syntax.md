...
!!! tip "值得记住的几个简写"
    ...
    - **列表项**：`标题 | 链接 | 描述 | 图标`，用竖线分隔，缺失的字段留空即可。
      这个顺序对**所有**列表类区块都一样（`steps` / `links` / `cards` / `features`……），
      不需要记每一块自己的顺序；操作按钮是 `文字 | 链接 | 样式 | 图标`。
    - **图标或图片**：`icon:` 写图标名就是字形，写以图片扩展名结尾的路径就是圆形图片。
      详见[图标与图片](blocks.md#_2)。
...
| `gap` | 长度 | 栅格间距 |
| `columns` | 整数 | 列数；**尺寸按这个列数算**，放不下时才自动减列 |
| `min_cols` | 长度 | 没写 `columns` 时单个栅格项的最小宽度，默认 `13em` |
| `ratio` | 比例 | 该区块图片的默认宽高比（如 `16/10`）。比例写**斜杠** |

`title` / `subtitle` / `eyebrow` 在大多数区块里都可用，支持行内 Markdown。

### 列表里的空位

所有可以重复的列表（`cards`、`features`、`stats`、`steps`、`links`、`testimonials`、
`logos`、`showcase`、`gallery`）都允许留空。空位占住自己的位置，但什么都不画：
...
`-`、`- {}`、`- empty`、`- blank`、`- gap`、`- spacer` 都写成空位。只有*同时*
带上分隔符才是一条真条目，所以 `- empty | /guide/` 是一张标题叫 empty 的卡片。

**列数不变**：4 列里只放 3 张卡，卡片宽度仍然是四列的宽度，第 4 个位置是空气——
不会因为少一张而把三张撑宽。

### 比例要写斜杠

`ratio: 4/3` 或者 `ratio: "4:3"`。**不要写不带引号的 `ratio: 4:3`**：
YAML 会把它当成六十进制数（`4×60+3 = 243`），而 243 是合法的 CSS 比例，
于是卡片会高到 243 倍宽。插件会认出这种值、在构建时报警，并回退到默认比例。
